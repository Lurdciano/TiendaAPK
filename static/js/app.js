  ...
  navItems.forEach((item) => {
    item.addEventListener("click", (e) => {
      ...
      if (targetId === "productos") {
        loadProductos();
        document.getElementById("fab-productos").style.display = "flex";
        document.getElementById("fab-clientes").style.display = "none";
      } else if (targetId === "clientes") {
        loadClientes();
        document.getElementById("fab-productos").style.display = "none";
        document.getElementById("fab-clientes").style.display = "flex";
      } else {
        document.getElementById("fab-productos").style.display = "none";
        document.getElementById("fab-clientes").style.display = "none";
...
function initDB() {
  return new Promise((resolve, reject) => {
    const request = indexedDB.open("TiendaRopaStandalone", 2);
    ...
    request.onupgradeneeded = (e) => {
      ...
      if (!db.objectStoreNames.contains("cuentas_corrientes")) {
        db.createObjectStore("cuentas_corrientes", { keyPath: "cliente_id" });
      }
      if (!db.objectStoreNames.contains("movimientos_cc")) {
        db.createObjectStore("movimientos_cc", {
          keyPath: "id",
          autoIncrement: true,
        });
      }
      if (!db.objectStoreNames.contains("objetivo_mensual")) {
        db.createObjectStore("objetivo_mensual", { keyPath: "mes_anio" });
      }
...
function initDefaults() {
  dbGet("configuracion", "markup_efectivo").then((val) => {
    if (!val)
      dbUpdate("configuracion", { clave: "markup_efectivo", valor: 30 });
  });
  dbGet("configuracion", "recargo_tarjeta").then((val) => {
    if (!val)
      dbUpdate("configuracion", { clave: "recargo_tarjeta", valor: 10 });
  });
}
...
// ===== CONFIGURACION Y OBJETIVOS =====
async function abrirModalConfig() {
  const markup = await dbGet("configuracion", "markup_efectivo");
  const recargo = await dbGet("configuracion", "recargo_tarjeta");

  document.getElementById("cfg-markup").value = markup ? markup.valor : 30;
  document.getElementById("cfg-recargo").value = recargo ? recargo.valor : 10;

  const hoy = new Date();
  const mes_anio = hoy.getFullYear() + "-" + (hoy.getMonth() + 1);
  const objetivo = await dbGet("objetivo_mensual", mes_anio);

  document.getElementById("cfg-objetivo").value = objetivo ? objetivo.meta : 0;

  new bootstrap.Modal(document.getElementById("modalConfig")).show();
}

async function guardarConfig() {
  const markup = document.getElementById("cfg-markup").value;
  const recargo = document.getElementById("cfg-recargo").value;
  const metaObj = parseFloat(
    document.getElementById("cfg-objetivo").value || 0,
  );

  await dbUpdate("configuracion", {
    clave: "markup_efectivo",
    valor: parseFloat(markup),
  });
  await dbUpdate("configuracion", {
    clave: "recargo_tarjeta",
    valor: parseFloat(recargo),
  });

  const hoy = new Date();
  const mes_anio = hoy.getFullYear() + "-" + (hoy.getMonth() + 1);
  const objetivoActual = await dbGet("objetivo_mensual", mes_anio);

  const objFinal = {
    mes_anio: mes_anio,
    meta: metaObj,
    acumulado: objetivoActual ? objetivoActual.acumulado : 0,
  };
  await dbUpdate("objetivo_mensual", objFinal);
...
async function agregarAObjetivo(monto) {
  if (monto === 0) return;
  const hoy = new Date();
  const mes_anio = hoy.getFullYear() + "-" + (hoy.getMonth() + 1);
  let obj = await dbGet("objetivo_mensual", mes_anio);
  if (!obj) {
    obj = { mes_anio: mes_anio, meta: 0, acumulado: 0 };
  }
  obj.acumulado += monto;
  await dbUpdate("objetivo_mensual", obj);
}
...
async function loadDashboard() {
  try {
    ...
    document.getElementById("dash-productos-total").textContent =
      productos.length;
    document.getElementById("dash-stock-bajo").textContent = productos.filter(
      (p) => p.stock <= 5,
    ).length;
    ...
    // Objetivo Mensual
    const dhoy = new Date();
    const mes_anio = dhoy.getFullYear() + "-" + (dhoy.getMonth() + 1);
    let obj = await dbGet("objetivo_mensual", mes_anio);
    if (!obj) obj = { meta: 0, acumulado: 0 };

    document.getElementById("dash-objetivo-monto").textContent =
      `$${obj.acumulado.toFixed(2)}`;
    const pct =
      obj.meta > 0 ? Math.min(100, (obj.acumulado / obj.meta) * 100) : 0;
    document.getElementById("dash-objetivo-pct").textContent =
      `${pct.toFixed(1)}%`;
    document.getElementById("dash-objetivo-barra").style.width = `${pct}%`;
  } catch (e) {
...
async function calcularPrecioSugerido() {
  ...
  if (costo > 0) {
    const confMarkup = await dbGet("configuracion", "markup_efectivo");
    const mkp = confMarkup ? confMarkup.valor : 30;
    const sugerido = costo * (1 + mkp / 100);
    document.getElementById("prod-precio").value = sugerido.toFixed(2);
  }
}
...
async function initVenta() {
  try {
    ...
    console.error("Error al cargar listados para venta", e);
  }
}
...
function renderVentaItems() {
  ...
  document.getElementById("venta-total-monto").textContent =
    `$${total.toFixed(2)}`;
}
...
async function procesarVenta() {
  ...
  try {
    ...
    if (metodo === "cuenta corriente" && cliente_id) {
      let cc = await dbGet("cuentas_corrientes", cliente_id);
      if (!cc) {
        cc = {
          cliente_id: cliente_id,
          saldo: 0,
      ...
      cc.saldo += total;
      cc.fecha_actualizacion = new Date().toISOString();
      await dbUpdate("cuentas_corrientes", cc);

      await dbAdd("movimientos_cc", {
        cliente_id: cliente_id,
        tipo: "cargo",
        monto: total,
    ...
    // Siempre suma al objetivo (asumiendo política del backend)
    await agregarAObjetivo(total);
...
async function loadClientes() {
  ...
  try {
    const clientes = await dbGetAll("clientes");
    const cuentas_corrientes = await dbGetAll("cuentas_corrientes");
    ...
    clientes.forEach((c) => {
      const cc = cuentas_corrientes.find((cc) => cc.cliente_id === c.id);
      const saldoDeuda = cc ? cc.saldo : 0;
      const badgeHtml =
        saldoDeuda > 0
          ? `<span class="deuda-badge mt-1 d-inline-block">Deuda $${saldoDeuda.toFixed(2)}</span>`
          : "";

      const item = document.createElement("div");
      item.className = "mobile-list-item client-item-container";
      item.onclick = () => abrirDetalleCliente(c.id);
      item.innerHTML = `
                <div class="mobile-list-info">
...
async function abrirDetalleCliente(id) {
  ...
  const cc = await dbGet("cuentas_corrientes", id);
  const movimientos = await dbGetAll("movimientos_cc");
  const movsCli = movimientos.filter((m) => m.cliente_id === id).reverse(); // Mas recientes primero

  document.getElementById("det-cli-nombre").textContent = cliente.nombre;
  const saldoPendiente = cc ? cc.saldo : 0;
  document.getElementById("det-cli-saldo").textContent =
    `$${saldoPendiente.toFixed(2)}`;
  document.getElementById("form-pago-cc").style.display = "none";
  document.getElementById("cc-monto-pago").value = "";

  let movsHtml = "";
  if (movsCli.length === 0) {
    movsHtml = '<p class="text-muted text-center mt-3">No hay movimientos.</p>';
  } else {
    movsCli.forEach((m) => {
      const f = new Date(m.fecha);
      const isAbono = m.tipo === "abono";
      const color = isAbono ? "text-success" : "text-danger";
      const sign = isAbono ? "-" : "+";
      movsHtml += `
            <div class="d-flex justify-content-between align-items-center py-2 border-bottom">
                <div>
                    <span class="d-block fw-bold">${m.descripcion}</span>
                    <span class="text-muted" style="font-size: 0.75rem;">${f.toLocaleDateString()}</span>
                </div>
                <div class="${color} fw-bold">${sign}$${m.monto.toFixed(2)}</div>
            </div>`;
    });
  }
  document.getElementById("lista-movimientos-cc").innerHTML = movsHtml;

  new bootstrap.Modal(document.getElementById("modalDetalleCliente")).show();
}

function mostrarFormPago() {
  document.getElementById("form-pago-cc").style.display = "block";
}

async function registrarPagoCC() {
  if (!clienteSeleccionadoId) return;
  const monto = parseFloat(document.getElementById("cc-monto-pago").value);
  if (!monto || monto <= 0) return alert("Monto inválido");

  let cc = await dbGet("cuentas_corrientes", clienteSeleccionadoId);
  if (!cc || cc.saldo <= 0) return alert("No hay deuda pendiente.");

  const montoReal = Math.min(monto, cc.saldo);
  ...
  await dbAdd("movimientos_cc", {
    ...
    tipo: "abono",
    monto: montoReal,
    descripcion: "Abono/Pago registrado manualmente",
    fecha: new Date().toISOString(),
  });

  await agregarAObjetivo(montoReal); // Suma al objetivo

  alert(`Pago de $${montoReal.toFixed(2)} procesado`);
